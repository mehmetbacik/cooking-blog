import type { TastyRecipeItem } from "../types";

export const tastyRecipesData: TastyRecipeItem[] = [
  {
    id: 1,
    slug: "chicken-meatballs-with-cream-cheese",
    title: "Chicken Meatballs with Cream Cheese",
    image: "/images/blog/tastyRecipes/tastyRecipe-1.png",
    author: "Andreas Paula",
    category: "Meat",
    time: "30 Minutes",
  },
  {
    id: 2,
    slug: "traditional-bolognaise-ragu",
    title: "Traditional Bolognaise Ragu",
    image: "/images/blog/tastyRecipes/tastyRecipe-2.png",
    author: "Andreas Paula",
    category: "Meat",
    time: "30 Minutes",
  },
  {
    id: 3,
    slug: "pork-and-chive-chinese-dumplings",
    title: "Pork and Chive Chinese Dumplings",
    image: "/images/blog/tastyRecipes/tastyRecipe-3.png",
    author: "Andreas Paula",
    category: "Meat",
    time: "30 Minutes",
  },
];
